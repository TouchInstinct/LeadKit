...
unless git.commits.all? { |c| c.message =~ valid_commit_style || c.message =~ merge_commit_style }
  fail("Please, run script setup.sh in framework's folder:  
```ruby
./setup/sh
```  
Make sure, your commits confirm rules - [config-conventional](https://github.com/conventional-changelog/commitlint/tree/master/%40commitlint/config-conventional#rules).  
